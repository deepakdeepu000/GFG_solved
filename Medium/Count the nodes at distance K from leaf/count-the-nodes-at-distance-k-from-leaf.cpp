...
class Solution
{
    ...
    vector<int> v;
    void fun(Node* root, int k,int level,set<Node*>& ans,vector<Node*> &path){
         if(root==NULL) return ;
        path.push_back(root);
        ...
        fun(root->left,k,level+1,ans,path);
        fun(root->right,k,level+1,ans,path);
        path.pop_back();
